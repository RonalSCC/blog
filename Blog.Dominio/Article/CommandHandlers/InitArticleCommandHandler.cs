...
public class InitArticleCommandHandler(IEventStore eventStore) : ICommandHandler<ArticleCommands.InitArticle>
{
    ...
    private static void AssertTagLengthIsCorrect(ArticleCommands.InitArticle command)
    {
        if (command.Tags.Count == 0)
            throw new InitArticleException(InitArticle.DEBE_CONTENER_AL_MENOS_UN_TAG_DESCRIPTIVO);
    }

    private static void AssertIfTitleIsEmpty(string title)
    {
        if (string.IsNullOrEmpty(title))
            throw new InitArticleException(InitArticle.EL_TITULO_NO_PUEDE_SER_VACIO);
    }

    private static void AssertIfLengthOfBlocksIsCorrect(List<object> blocks)
    {
        if (blocks.Count == 0)
            throw new InitArticleException(InitArticle.DEBE_CONTENER_AL_MENOS_UN_BLOQUE);

        if (blocks.Count > 20)
            throw new InitArticleException(InitArticle.NO_PUEDE_TENER_MAS_DE_20_BLOQUES);
    }

    private static void AssertIfLengthOfAuthorsIsCorrect(List<object> authors)
    {
        if (authors.Count == 0)
            throw new InitArticleException(InitArticle.DEBE_CONTENER_AL_MENOS_UN_AUTOR);
    }
}